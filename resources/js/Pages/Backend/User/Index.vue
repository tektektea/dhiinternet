<template>
    <q-page class="container" padding>
        <q-breadcrumbs>
            ...
            <q-breadcrumbs-el label="User Accounts" />
        </q-breadcrumbs>
        <br/>
        <div class="flex justify-between items-end q-pa-md bg-white">
            <div>
                <div class="text-lg text-bold">User Accounts</div>
                <br/>
                <q-input v-model="state.filter"
                         placeholder="Search"
                         outlined
                         bg-color="grey-2"
                         dense>
            ...
            <div>
                <q-btn class="sized-btn" @click="$inertia.get(route('user.create'))" color="primary" label="New User" no-caps/>
            </div>
        </div>
            ...
            <div @click="e=>openBooking(item)"  v-for="(item,index) in list.data" :key="index">
                <div class="flex justify-between items-center q-mt-sm cursor-pointer">
                    ...
                    <div class="flex items-center q-gutter-sm">
                        <q-chip v-for="role in item?.roles" square :label="role?.name"/>
                        <q-separator vertical/>
                        <q-btn size="sm" @click="$inertia.get(route('user.edit',item.id))" outline color="secondary" icon="o_edit"/>
                        <q-btn size="sm" @click="handleDelete(item)" color="negative" outline icon="o_delete"/>
                    </div>
                </div>
                <q-separator/>
