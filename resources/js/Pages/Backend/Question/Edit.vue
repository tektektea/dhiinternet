<template>
    <q-page class="container" padding>
        <q-breadcrumbs>
            ...
            <q-breadcrumbs-el @click="$inertia.get(route('question.index'))" label="FAQ" />
            <q-breadcrumbs-el @click="$inertia.get(route('question.edit',item.id))" label="Edit" />
        </q-breadcrumbs>
        <br/>
        <div class="flex justify-between items-center bg-white q-pa-md">
            <div class="text-lg text-dark text-weight-medium">{{item.question}}</div>
        </div>
        <br/>
        <q-form @submit="handleSubmit" class="column  bg-white q-pa-md">
            <q-input v-model="form.question"
                     ...
                     :error-message="form?.errors?.question?.toString()"
                     :rules="[
...
const handleSubmit=e=>{
    form.put(route('question.update',props.item.id),{
